...
class _ExpenseVoucherScreenState extends State<ExpenseVoucherScreen> {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            Text('₹$amount', style: AppTypography.headlineMedium.copyWith(fontWeight: FontWeight.w900, color: AppColors.error)),
                          ],
                        ),
                      ),
                    ],
                  ...
                  AppSpacing.vMd,
                  Text('PAYMENT & RECIPIENT INFO', style: AppTypography.labelMedium.copyWith(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
                  AppSpacing.vSm,
                  ...
                  _buildDetailRow('Paid To:', voucher['paidTo'] ?? 'N/A', isDark),
                  if (voucher['bankName'] != null) _buildDetailRow('Bank Name:', voucher['bankName'] as String, isDark),
                  if (voucher['chequeNumber'] != null) _buildDetailRow('Cheque #:', voucher['chequeNumber'] as String, isDark),
  ...
  void _openFormDialog({Map<String, dynamic>? voucher}) {
    ...
    final numController = TextEditingController(text: voucher?['voucherNumber'] ?? 'EV-2024-00${_vouchers.length + 1}');
    ...
    final paidToController = TextEditingController(text: voucher?['paidTo'] ?? '');
    ...
    final chequeController = TextEditingController(text: voucher?['chequeNumber'] ?? '');
    final descController = TextEditingController(text: voucher?['description'] ?? '');
    String headId = voucher?['headId'] ?? '1';
    ...
    showDialog(
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          ...
          return AlertDialog(
            title: Text(
              isEditing ? 'Edit Expense Voucher' : 'Create Expense Voucher',
              style: AppTypography.titleLarge.copyWith(fontWeight: FontWeight.w700),
            ),
            content: SizedBox(
              width: 560,
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(child: AppTextField(controller: numController, label: 'Voucher Number *', hint: 'EV-2024-001')),
                        AppSpacing.hMd,
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                items: const [
                                  ...
                                  DropdownMenuItem(value: '4', child: Text('Other Expense')),
                                ],
                    ...
                    Row(
                      children: [
                        ...
                        AppSpacing.hMd,
                        Expanded(child: AppTextField(controller: paidToController, label: 'Paid To', hint: 'Staff Member A')),
                      ],
                    ),
                    if (isBank) ...[
                      AppSpacing.vMd,
                      Row(
                        children: [
                          Expanded(child: AppTextField(controller: bankController, label: 'Bank Name', hint: 'SBI')),
                          AppSpacing.hMd,
                          Expanded(
                            child: paymentMethod == 'cheque'
                                ? AppTextField(controller: chequeController, label: 'Cheque #', hint: 'CHQ987654')
                                : AppTextField(controller: refController, label: 'Reference #', hint: 'TXN123456789'),
                          ),
                        ],
                      ),
                    ],
                    ...
                    TextFormField(
                      ...
                      maxLines: 2,
                      decoration: const InputDecoration(hintText: 'Brief description of expense...'),
                    ),
                  ],
                ),
              ),
                ...
                onPressed: () {
                  ...
                  setState(() {
                    if (isEditing) {
                      ...
                      voucher['headName'] = headMap[headId] ?? 'Other Expense';
                      voucher['amount'] = amt;
                      voucher['paymentMethod'] = paymentMethod;
                      voucher['paidTo'] = paidToController.text.trim();
                      voucher['bankName'] = bankController.text.trim();
                      voucher['chequeNumber'] = chequeController.text.trim();
                      ...
                      _vouchers.add({
                        ...
                        'headId': headId,
                        'headName': headMap[headId] ?? 'Other Expense',
                        'amount': amt,
                        'paymentMethod': paymentMethod,
                        'paidTo': paidToController.text.trim(),
                        'bankName': bankController.text.trim(),
                        'chequeNumber': chequeController.text.trim(),
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(isEditing ? 'Expense voucher updated!' : 'Expense voucher created!'),
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _vouchers.where((v) {
      if (_searchQuery.isNotEmpty) {
        ...
        final paid = (v['paidTo'] as String? ?? '').toLowerCase();
        return num.contains(q) || head.contains(q) || paid.contains(q);
      }
      return true;
    }).toList();
                      ...
                      child: ConstrainedBox(
                        ...
                        child: DataTable(
                          ...
                          columns: [
                            ...
                            _buildDataColumn('PAID TO', isDark),
                          ...
                          rows: filtered.map((voucher) {
                            ...
                            return DataRow(
                              cells: [
                                ...
                                DataCell(Text('$paid', style: AppTypography.bodySmall)),
                                DataCell(
    ...
    final headerTexts = Column(
      ...
      children: [
        Row(
          children: [
            InkWell(
              onTap: () => context.go(RouteNames.expenseExpenseVoucherPath),
              child: Text(
                'Expense',
                style: AppTypography.bodySmall.copyWith(
                  color: isDark ? AppColors.textMutedDark : AppColors.textMutedLight,
